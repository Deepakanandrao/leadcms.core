// <copyright file="TestOpenAIProviderService.cs" company="WavePoint Co. Ltd.">
...
public class TestOpenAIProviderService : IAIProviderService
{
    ...
    public Task<TextGenerationResponse> GenerateTextAsync(TextGenerationRequest request)
    {
        ...
        {
            GeneratedText = $"[TestOpenAI] {request.UserPrompt}",
    ...
    private static void RecordAndLog(string requestType, RecordedRequest record, string logBody)
    {
        lock (LogLock)
        {
            ...
            try
            {
                ...
                var entry = string.Join(System.Environment.NewLine, header, logBody, "==== End Request ====", string.Empty);
